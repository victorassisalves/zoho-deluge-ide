    ...
    try {
        ...
        Bus.listen(MSG.CODE_PULL + ':response', (payload) => {
            ...
            if (payload && payload.code) {
                if (editor) {
                    ...
                    saveToDexie(false); // Treat pulled code as "clean"
                }
        ...
        editor.onDidChangeModelContent(() => {
            ...
            if (currentContextHash) {
                ...
                autoSaveTimer = setTimeout(() => {
                    saveToDexie(true); // Auto-save sets dirty = true
                }, 1000);
            }
        });
        ...
        if (typeof chrome !== "undefined" && chrome.storage) {
            chrome.storage.local.get(['saved_deluge_code', 'theme', 'activation_behavior', 'json_mappings', 'left_panel_width', 'right_sidebar_width', 'bottom_panel_height', 'font_size'], (result) => {
                // We defer code loading to handleContextSwitch if we are connected,
                // but if offline, we load the legacy backup or simple default.
                if (!isConnected && result.saved_deluge_code) editor.setValue(result.saved_deluge_code);
...
async function saveToDexie(isDirty = true) {
    ...
    try {
        await db.files.put({
            id: currentContextHash,
            workspaceId: currentContext.orgId || currentContext.service,
            fileName: fileName,
            code: code,
            variables: extractVarsFromCode(code),
            lastSaved: Date.now(),
            isDirty: isDirty
        });
        ...
        if (!isDirty) {
            ...
            if (syncEl) syncEl.innerText = 'Auto-saved ' + new Date().toLocaleTimeString();
        }
    } catch (e) {
        console.error('[ZohoIDE] DB Save Error:', e);
    }
}
...
function setupEventHandlers() {
    ...
    bind('save-btn', 'click', () => saveToDexie(false)); // Manual Save button now saves to Dexie
